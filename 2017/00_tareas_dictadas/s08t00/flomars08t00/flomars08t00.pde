...
int x, y, l; //se declaran las variables
float z, a, b, c, d, e, f, g, x1, y1, x2, y2, q, w; //se declaran las variables
...
void draw(){ //inicio ámbito de dibujo
  ...
  triangulos(x1, y1, x2, y2, colores [1], colores [0], colores [2]); //función 3
   ...
   for (int i = 10; i < width - mouseX ; i = i+10) {
    for (int j = 15; j < height - mouseY ; j = j +10) {
      stroke(colores[6]);
      strokeWeight(3);
      point(i, j);
