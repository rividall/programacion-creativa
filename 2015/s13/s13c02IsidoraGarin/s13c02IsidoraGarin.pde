...
void setup() {
  ...
  beginRecord(PDF, "Visualizacion_Filadelfia.pdf");
  ...
  datos = loadStrings("filadelfia.csv");
  ...
  for (int x = 0; x < cols; x++) {
    for (int y = 0; y < filas; y++) {
      ...
      if ((nums >= 0) && (nums < 365)) {
        ...
        float vismax = float(dia[nums][13]);
        float vismin = float(dia[nums][15]);
        float velviento = float(dia[nums][16]);
        float precip = float(dia[nums][19]);
        float cloud = float(dia[nums][20]);
        float dirviento = float(dia[nums][22]);

        //FUNCION
        visualizacion(x1, y1, temmax, temmed, temmin, rocio, hummax, hummin, vismax, vismin, velviento, precip, cloud, dirviento);
      }
    }
  }
  endRecord();
