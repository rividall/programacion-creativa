class Clase {
	...
	Clase(String mes, String dia, float maxima,float media, float minima, float minHum, float maxHum, float meanHum, float promanualtmax, float promanualtmin, float promtmeanmes,float dirViento,float dewmax, float dewmean, float dewmin, float meanviento, int evento, float tmaxmes,float tminmes) {
		this.mes = mes;
		this.dia = dia;
  ...
  // crea lineas las cuales muestran  cuanto es la media de temperaturas por dia en un año
	//Los colores azul representa lluvia, el verde nada,el rojo neblina y el rosa o magenta representa lluvia y neblina.
...
//Se visualizan ellipses con posición en el eje y  que varía según las max temperaturas y su opacidad varía según la humedad del lugar en meses
    ...
    txt();
		pushMatrix();
		translate(width/2,height/2);
		for(int u= 0; u < totdias; u++ ){// el número de esferas equivale a los 365 dias del año
		...
		for(int e = 0; e<dewmax; e++){//lineas representando lluvia
		rotate(dirViento);
		strokeWeight(10);
		line(x,y,x+5,y+5);
		x += dirViento/5;
